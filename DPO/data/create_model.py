...
def create_model(model:str, provider: str):
    ...
    if provider == "google":
        ...
        if not model.startswith("openai/"):
            model = "openai/" + model
        return LiteLlm(model=model, api_key=os.environ.get("BAICHUAN_API_KEY"), api_base="https://api.baichuan-ai.com/v1")
        ...
        # huggingface的模型需要使用LiteLlm
        assert os.environ.get("SGLANG_API_KEY"), "SGLANG_API_KEY is not set"
        assert os.environ.get("SGLANG_API_URL"), "SGLANG_API_URL is not set"
        if not model.startswith("openai/"):
            # 表示兼容openai的模型请求
            model = "openai/" + model
        return LiteLlm(model=model, api_key=os.environ.get("SGLANG_API_KEY"), api_base=os.environ.get("SGLANG_API_URL"))
